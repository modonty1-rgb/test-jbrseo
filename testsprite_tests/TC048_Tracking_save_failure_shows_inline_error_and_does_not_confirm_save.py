...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /admin/login (use exact path: http://localhost:3000/admin/login)
        await page.goto("http://localhost:3000/admin/login", wait_until="commit", timeout=10000)
        
        # -> Type the password into the visible password input (index 2725) and click the Log in button (index 2728) to attempt to authenticate and continue to the admin area.
